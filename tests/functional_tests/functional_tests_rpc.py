...
# a main offline qsfd, does most of the tests
# a restricted RPC qsfd setup with RPC payment
# two local online qsfds connected to each other
N_qsfDS = 4

# 4 wallets connected to the main offline qsfd
# 1 wallet connected to the first local online qsfd
# 1 offline wallet
N_WALLETS = 6
...
qsfd_base = [builddir + "/bin/qsfd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "qsfd_p2p_port", "--rpc-bind-port", "qsfd_rpc_port", "--zmq-rpc-bind-port", "qsfd_zmq_port", "--zmq-pub", "qsfd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "qsfd_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
qsfd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/qsf-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_qsfDS):
  command_lines.append([str(18180+i) if x == "qsfd_rpc_port" else str(18280+i) if x == "qsfd_p2p_port" else str(18380+i) if x == "qsfd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "qsfd_zmq_pub" else builddir + "/functional-tests-directory/qsfd" + str(i) if x == "qsfd_data_dir" else x for x in qsfd_base])
  if i < len(qsfd_extra):
    command_lines[-1] += qsfd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/qsfd' + str(i) + '.log', 'a+'))
