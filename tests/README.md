...
Core tests take longer than any other QSF tests, due to the high amount of computational work involved in validating core components.
...
To run only QSF's core tests (after building):
...
To run only QSF's crypto tests (after building):
...
```bash
qsfd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `qsf-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only QSF's hash tests (after building):
...
To run only QSF's performance tests (after building):
...
To run only QSF's unit tests (after building):
