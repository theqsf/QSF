...
The intended audience are developers, who want to leverage newly added tricks to QSF via `CMake`. The document will lower the entry point for these developers.
...
QSF can be compiled via the main `Makefile`, using one of several targets listed there.
...
It is possible to extract these `CMake` calls and modify them for your specific needs. For example, a minimal external cmake command to compile QSF, executed from within a newly created build directory could look like:

`cmake -S "$DIR_SRC" -DCMAKE_BUILD_TYPE=Release && make`

where the variable `DIR_SRC` is expected to store the path to the QSF source code.
...
The additional artifact of the above call is the `qsf.cbp` Code::Blocks project file in the build directory.
...
After a successful build, open the `qsf.cbp` with CB. From the CB's menu bar select the target, that you want debug. Assuming these are unit tests:
...
https://github.com/qsf-project/qsf/pull/7160

### Faster core_tests with caching
https://github.com/qsf-project/qsf/pull/5821

### Precompiled headers
https://github.com/qsf-project/qsf/pull/7216

### Unity builds
https://github.com/qsf-project/qsf/pull/7217
