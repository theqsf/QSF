# Anonymity Networks with QSF

Currently only Tor and I2P have been integrated into QSF. The usage of
...
is made or until `qsfd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `qsf-wallet-cli` and
`qsf-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `qsfd` the ip/port of a socks proxy provided by a
...
which tells `qsfd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `qsfd` the inbound address, network
...
which tells `qsfd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `qsfd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p" and
forwarded to `qsfd` localhost port 30000.
...
`qsfd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/qsf
...
This will store key information in `/var/lib/tor/data/qsf` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/qsf/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the QSF network should have
similar timestamps.
...
### Intermittent QSF Syncing

If a user only runs `qsfd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `qsfd` as often as possible to conceal when transactions are being sent.
...
`qsfd` currently selects two outgoing connections every 5 minutes for
...
`qsfd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `qsfd` should likely disconnect
