...
used by QSF. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in QSF's
...
> This document does not currently list all data being sent by the qsf
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about QSF p2p traffic!


## Header
This header is sent for every QSF p2p message.
...
QSF when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the QSF specific
...
### Requests
Requests are the basis of the admin protocol for QSF. The `Q` bit must be
