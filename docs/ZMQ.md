# The Current/Future Status of ZMQ in QSF

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. QSF
...
matching supported by both QSF and ZMQ. The `format`, `context` and `event`
...
The QSF daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The QSF daemon also ensures that
...
The QSF daemon will send a `txpool_add` pub exactly once for each
