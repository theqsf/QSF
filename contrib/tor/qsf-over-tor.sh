...
"$qsfd" \
  ...
  --add-priority-node 2qsfnode5itf65lz.onion:18083 \
  --detach
ready=0
for i in `seq 10`
do
...
if test "$ready" = 0
then
  echo "Error starting qsfd"
  tail -n 400 "$HOME/.bitqsf/bitqsf.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*QSF\ \'" | tail -n 20
  exit 1
fi
