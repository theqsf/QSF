...
gsigs = 'https://github.com/qsf-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('qsf'):
        # Remove the potentially stale qsf dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'qsf', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'qsf'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/qsf/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'qsf='+args.commit, '--url', 'qsf='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/qsf-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/qsf/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/qsf/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/qsf-project/qsf', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/qsf', exist_ok=True)
    os.chdir('builder/inputs/qsf')
    if args.pull:
